...
readlen = 75
readspertx = round(1 * as.numeric(width(fasta)) / readlen) * 10
readspertx[which(readspertx %in% 0)] = 1
...
#	readspertx[i] = max(round(as.numeric(theoretical[i, "NumReads"])), 1)#*as.numeric(args[4])
#}
#print(theoretical[2, "NumReads"])
#print(readspertx[2])
...
simulate_experiment(args[1], reads_per_transcript=readspertx, num_reps=c(1,1), fold_changes=fold_changes, error_model='illumina5', 
	bias='cdnaf', outdir=args[2])
#simulate_experiment(args[1], mean_model=TRUE, num_reps=c(1,1), fold_changes=fold_changes, error_model='illumina5', 
