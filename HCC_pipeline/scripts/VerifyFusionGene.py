...
class SV_t(object):
	...
	def Print(self):
		...
		print(strbp1+"\t|\t"+strbp2)+"\t"+self.fg
	def ToString(self):
		strbp1=self.BP1.ToString()
		strbp2=self.BP2.ToString()
		return strbp1+"\t|\t"+strbp2+"\t"+self.fg
...
def SQUIDAccuracy(inputfile, outputfile, SVs):
	...
	for line in fpin:
		...
		if hit:
			hitcount+=1
			fpout.write("TP\t"+sv.fg+"\t"+line)
		else:
			fpout.write("FP\t"+"Nan\t"+line)
	fpin.close()
	fpout.close()
...
if __name__=="__main__":
	...
	if len(sys.argv)!=4:
		...
		TrueSV=sys.argv[2]
		suffix=os.path.basename(TrueSV).split(".")[0].split("_")[-1]
		pred=sys.argv[3]
		start=pred.rfind(".")
		out=pred[:start]+"_"+suffix+"_hit"+pred[start:]
